...
def generate_lt_param_combos(segments = [6,8,10,11],spike = [0.75,0.90,1.0],recovery = [0.25,0.50,0.90,1.0],pValue = [0.05,0.10,0.15]): 
    ...
    # asign each parameter to template 
        ...
        ltParamTemplate = {"timeSeries": ee.ImageCollection([]), "maxSegments": subList[0], "spikeThreshold": subList[1], "vertexCountOvershoot": 3, "preventOneYearRecovery": True, "recoveryThreshold": subList[2], "pvalThreshold": subList[3], "bestModelProportion": 0.75, "minObservationsNeeded": subList[0], "param_num": counter}
